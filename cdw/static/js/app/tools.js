...
window.VerifyPhoneView = Backbone.View.extend({
    ...
    initialize : function() {
        ...
        $('.phone3, .phone4').bind('keyup keydown blur', function(e) {
            $('form input[name=phonenumber]').val($('input[name=areacode]').val() + $('input[name=firstthree]').val() + $('input[name=lastfour]').val());
        });
    ...
    showMessage : function(msg) {
        this.$('.verify-msg').stop(true, true).text(msg).show().delay(3000).fadeOut();
    }
});
...
window.RegisterView = Backbone.View.extend({
    ...
    updateCharsLeft : function(e) {
        if (this.$usernameInput.val().length > 18) {
            this.$usernameInput.val(this.$usernameInput.val().slice(0, 18));
        }
        this.$('span.chars-left').text(18 - this.$usernameInput.val().length)
    },
