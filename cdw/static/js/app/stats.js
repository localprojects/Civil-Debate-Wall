...
window.StatsScreenView = Backbone.View.extend({
    ...
    initialize : function() {
        ...
        window.StatsMostDebated = new StatsMostDebatedView({
            el : $('div.most-debated'),
            model : new Backbone.Collection(data.mostDebatedOpinions)
        });

        window.StatsMostLiked = new StatsMostLikedView({
            el : $('div.most-liked'),
            model : new Backbone.Collection(data.mostLikedOpinions)
        });

        window.StatsFrequentWords = new StatsFrequentWordsView({
    ...
    render : function() {
        ...
        var yesWidth = Math.max(20, Math.min(80, Math.floor(100 * (data.debateTotals.yes / totalAnswers))));
        var noWidth = 100 - yesWidth;

        this.$('div.opinions-bar .yes-bar').css({
            width : yesWidth + '%'
        });
        this.$('div.opinions-bar .no-bar').css({
            width : noWidth + '%'
        });
...
window.StatsMostDebatedView = Backbone.View.extend({
    ...
    addOne : function(item, index) {
        ...
        var view = new StatsMostDebatedDetailView({
            model : item
        });
        this.$('div.detail-view ul').append(view.render().el);
        view.adjustRag();

        view = new StatsMostDebatedMenuView({
            model : item
        });
...
window.StatsMostLikedView = Backbone.View.extend({
    ...
    addOne : function(item, index) {
        ...
        var view = new StatsMostLikedDetailView({
            model : item
        });
        this.$('div.detail-view ul').append(view.render().el);

        view = new StatsMostLikedMenuView({
            model : item
        });
        this.$('div.menu-view ul').append(view.render().el);
    },
...
window.StatsFrequentWordsView = Backbone.View.extend({
    ...
    render : function() {
        ...
        this.$('button').click($.proxy(function(e) {
            e.preventDefault();
        ...
        }, this)).mouseout($.proxy(function(e) {
            e.preventDefault();
            for (var i = 0; i < this.allButtons.length; i++) {
    ...
    showWordMenu : function(e) {
        ...
        _.each(this.detailPosts, function(item) {
            item.remove()
        });
        this.detailPosts = [];
