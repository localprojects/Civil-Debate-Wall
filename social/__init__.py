...
def get_cv(key):
    return getattr(current_app, current_app.config['SOCIAL'][key])

connection_service = LocalProxy(lambda: get_cv('connection_service_name'))
...
class Connection(object):
    ...
    def __init__(self, user_id, provider_id, provider_user_id, 
                 ...
                 profile_url=None, image_url=None, api=None, **kwargs):
...
class ConnectionService(object):
    """A ConnectionService handles the persistence and querying of connections 
    with SaaS providers to the local application's users.
    """
    
    def remove_connection(self, user_id, provider_id, 
                          provider_user_id, **kwargs):
        """Remove a single connection from a provider for the specified user
        """
        raise NotImplementedError("remove_connection")
    
    def remove_all_connections(self, user_id, provider_id, **kwargs):
        """Remove all connections from a provider for the specified user
        """
        raise NotImplementedError("remove_all_connections")
    
    def save_connection(self, user_id, provider_id, 
                        provider_user_id, access_token, secret, **kwargs):
        """Save a connection between a provider account and a local user account
        """
        raise NotImplementedError("save_connection")
    ...
    def get_connection_by_provider_user_id(self, provider_id, 
        ...
        """
        raise NotImplementedError("get_connection_by_provider_user_id")
    
    def get_primary_connection(self, user_id, provider_id, **kwargs):
        """Get the first connection found for the specified provider to the 
        currently logged in user.
        """
        raise NotImplementedError("get_current_user_connection")
    
    def get_connection(self, user_id, provider_id, provider_user_id, **kwargs):
        """Get a specific connection for the specified user, provider, 
        and provider user
        """
        raise NotImplementedError("get_connection")
...
def _connect_handler(connection_values, provider_id):
    ...
    try:
        ...
        current_app.logger.debug('Connection to %s established for '
                                 '%s' % (display_name, current_user))
        ...
        flash("A connection is already established with %s "
              "to your account" % display_name)
        
    except Exception, e:
        current_app.logger.error('Unexpected error connecting %s '
                                 'account for user %s. Reason: '
                                 '%s' % (display_name, current_user, e))
        flash("Could not make connection to %s. "
              "Please try again later." % display_name)
    
    redirect_url = session.get('post_oauth_connect_url', 
                               get_cv('connect_allow_view'))
...
class ConnectionFactory(object):
    ...
    def _get_primary_connection(self, user_id):
        return connection_service.get_primary_connection(user_id, 
                                                         self.provider_id)
    
    def _get_specific_connection(self, user_id, provider_user_id):
        return connection_service.get_connection(user_id, 
                                                 self.provider_id, 
                                                 provider_user_id)
...
class FacebookConnectionFactory(ConnectionFactory):
    def __init__(self, **kwargs):
        super(FacebookConnectionFactory, self).__init__('facebook', 
            kwargs['login_handler'], kwargs['connect_handler'])
...
class LoginHandler(OAuthHandler):
    ...
    def __call__(self, response):
        ...
        current_app.logger.debug('Received login response from %s. '
...
class FacebookLoginHandler(LoginHandler):
    """FacebookLoginHandler handles the authorization response from Facebook. 
    The Facebook account's user ID is not passed in the response, thus it must 
    be retrieved with an API call. This handler is dependent on the Python 
    Facebook library to perform the API call.
    """
    def __init__(self, **kwargs):
...
class ConnectHandler(OAuthHandler):
    ...
    def get_connection_values(self, response):
        raise NotImplementedError("get_connection_values method "
                                  "not implemented")
    
    def __call__(self, response):
        display_name = get_display_name(self.provider_id)
        current_app.logger.debug('Received connect response from %s. '
                                 '%s' % (display_name, response))
        
        if response is None:
            flash("Access was denied by %s" % display_name)
            return redirect(get_cv('connect_deny_view'))
...
class TwitterConnectHandler(ConnectHandler):
    """TwitterConnectHandler handles the connection procedure after a user 
    authorizes a connection from Twitter. The connection values are all 
    retrieved from the response, no extra API calls are necessary.
    """
    def __init__(self, **kwargs):
...
class FacebookConnectHandler(ConnectHandler):
    """FacebookConnectHandler handles the connection procedure after a user 
    authorizes a connection from Facebook. The Facebook acount's user ID is 
    retrieved via an API call, otherwise the token is provided by the response 
    from Facebook.
    """
    def __init__(self, **kwargs):
...
class Social(object):
    """Social adds integration with various SaaS providers to your application. 
    Currently Twitter and Facebook are supported. When properly configured, 
    Social will add endpoints to your app that allows for users to login via 
    SaaS providers and connect their SaaS provider accounts with a local account.
    """
    ...
    def init_app(self, app):
        ...
        if 'SOCIAL_PROVIDERS' in app.config:
            for provider, provider_config in default_provider_config.items():
                if provider in app.config['SOCIAL_PROVIDERS']:
                    ...
                    p_config = app.config['SOCIAL_PROVIDERS'][provider]['oauth']
        ...
        def login(provider_id):
            ...
            session['post_oauth_login_url'] = request.form.get('next', 
                current_app.config['AUTH']['post_login_view'])
        ...
        def connect(provider_id):
            ...
            current_app.logger.debug('Starting process of connecting %s '
                'account to user account %s. Callback URL = %s' % (
                    get_display_name(provider_id), current_user, callback_url))
            
            session['post_oauth_connect_url'] = request.form.get('next', 
                current_app.config['SOCIAL']['connect_allow_view'])
        ...
        def remove_connection(provider_id, provider_user_id):
            try:
                ...
                connection_service.remove_connection(
                    current_user.get_id(), provider_id, provider_user_id)
                ...
                current_app.logger.error('Unable to remove connection to %s/%s '
